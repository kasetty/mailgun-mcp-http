/**
 * @fileoverview Shared MCP server setup for Mailgun MCP Server
 * @description Contains the MCP server instance and tool registration logic
 *              that is shared between stdio and HTTP transports.
...
/**
 * Creates and configures a new MCP server instance with Mailgun tools
 *
 * @returns {McpServer} Configured MCP server instance with all Mailgun tools registered
 *
 * @example
...
export function createMcpServer() {
  ...
  generateToolsFromOpenApi(server, openApiSpec);
 ...
 * @throws {Error} When Mailgun API returns an error or response parsing fails
 */
export async function makeMailgunRequest(method, apiPath, data = null) {
 ...
 */
export function generateToolsFromOpenApi(server, openApiSpec) {
 ...
 * @throws {Error} When required path parameter is missing
 */
